...
Status LatController::Init(std::shared_ptr<DependencyInjector> injector,
  ...
  */
  matrix_a_(0, 1) = 1;// 1 2
  matrix_a_(1, 2) = (c_f + c_r) / m;// 2 3
  matrix_a_(2, 3) = 1;// 3 4
  matrix_a_(3, 2) =  (l_f * c_f - l_r * c_r) / i_z;// 4 3

  matrix_a_coeff_ = Matrix::Zero(matrix_size, matrix_size);
  matrix_a_coeff_(1, 1) = (-(c_f + c_r) / m) / v; // 2 2
  matrix_a_coeff_(1, 3) = (l_r * c_r - l_f * c_f) / m / v; // 2 4
  matrix_a_coeff_(3, 1) = ((lr * cr - lf * cf) / i_z) / v; // 4 2
  matrix_a_coeff_(3, 3) = (-1.0 * (l_f^2 * c_f + l_r^2 * c_r) / i_z) / v; // 4 4
...
Status LatController::ComputeControlCommand(
  ...
  */
  ...
  steer_angle = steer_angle_feedforwardterm + steer_angle_feedback_augment;
...
double LatController::ComputeFeedForward(double ref_curvature) const {
  ...
  if (injector_->vehicle_state()->gear() == canbus::Chassis::GEAR_REVERSE) {
    ...
    steer_angle_feedforwardterm =
        ((lf_ + lr_) * ref_curvature + kv * v * v * ref_curvature
         matrix_k_(0,2) *
             (lr_ / (2 * cf_) -
              lf_ * mass_ * v * v * ref_curvature / (2 * cr_ * ( lr_ + lf_)) )) *
